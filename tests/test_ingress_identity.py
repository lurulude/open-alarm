...
def _configure(data_dir: Path, monkeypatch) -> None:
...
def test_first_ingress_user_bootstraps_open_alarm_admin(tmp_path: Path, monkeypatch) -> None:
    data_dir = tmp_path / "first-user"
    _configure(data_dir, monkeypatch)
...
def test_later_ingress_user_defaults_to_open_alarm_viewer(tmp_path: Path, monkeypatch) -> None:
...
def test_ingress_source_gate_prevents_header_spoofing(monkeypatch) -> None:
